...
let
  cfg = config.horizon.services.samba;
  ...
  shares-submodule = with types; submodule ({ name, ... }: {
    options = {
      ...
      only-owner-editable = mkBoolOpt false "Whether the share is only writable by the system owner (horizon.user.name).";
...
in
{
  options.horizon.services.samba = with types; {
