  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message = "horizon.services.writefreely.database.passwordFile must be set if horizon.services.writefreely.database.createLocally is set to true";
      }
      {
        assertion = isSqlite -> !cfg.database.createLocally;
        message = "horizon.services.writefreely.database.createLocally has no use when horizon.services.writefreely.database.type is set to sqlite3";
      }
    ];
